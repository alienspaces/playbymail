...
import { getGameTurnSheetPreviewUrl } from '../api/gameImages';
import { useAuthStore } from '../stores/auth';

defineOptions({
    name: 'GameTurnSheetPreviewModal'
});
...
const previewUrl = computed(() => {
    console.log('[GameTurnSheetPreviewModal] Computing previewUrl, props.turnSheetType:', props.turnSheetType, 'props.visible:', props.visible);
    if (!props.gameId || !props.turnSheetType || !props.visible) return '';
    const baseUrl = getGameTurnSheetPreviewUrl(props.gameId, props.turnSheetType);
    ...
    console.log('[GameTurnSheetPreviewModal] Preview URL:', url);
    return url;
});
...
function startLoading() {
    console.log('[GameTurnSheetPreviewModal] Starting to load preview');
    loading.value = true;
    error.value = null;
}

function retryPreview() {
    console.log('[GameTurnSheetPreviewModal] Retrying preview');
    iframeKey.value++; // Force iframe to reload
    startLoading();
}

function onIframeLoad() {
    console.log('[GameTurnSheetPreviewModal] Iframe loaded successfully');
    loading.value = false;
}

function onIframeError() {
    console.log('[GameTurnSheetPreviewModal] Iframe load error');
    loading.value = false;
...
watch(() => props.visible, (newVal) => {
    if (newVal) {
        console.log('[GameTurnSheetPreviewModal] Modal opened, gameId:', props.gameId, 'turnSheetType:', props.turnSheetType);
        startLoading();
    } else {
