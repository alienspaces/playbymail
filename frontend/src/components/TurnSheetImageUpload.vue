<template>
  <div class="turn-sheet-image-upload">
    <h3 class="upload-title">Turn Sheet Background Image</h3>
    <p class="upload-description">
      Upload a background image for your game's turn sheets.
    ...
    <div class="image-section">
      ...
      <div class="upload-controls">
        <input id="file-background" type="file" accept="image/webp,image/png,image/jpeg" class="file-input"
          @change="handleFileSelect" :disabled="loading" />
        <label for="file-background" class="upload-btn">
...
async function loadImage() {
  ...
  try {
    const response = await getGameTurnSheetImage(props.gameId);
    ...
    console.error('Failed to load turn sheet image:', err);
    image.value = null;
  }
}
...
async function handleFileSelect(event) {
  ...
  loading.value = true;
  // Immediately emit loading state change (watch may not fire synchronously)
  emit('loadingChanged', true);
  console.log(`[TurnSheetImageUpload] Starting upload for gameId: ${props.gameId}`);

  try {
    const response = await uploadGameTurnSheetImage(props.gameId, file);
    console.log(`[TurnSheetImageUpload] Upload response:`, response);
    if (response.data) {
    ...
    console.error(`[TurnSheetImageUpload] Upload failed:`, err);
    error.value = err.message || 'Failed to upload image';
  } finally {
    loading.value = false;
    // Emit the computed loading state
    console.log(`[TurnSheetImageUpload] Upload complete`);
...
async function removeImage() {
  ...
  try {
    await deleteGameTurnSheetImage(props.gameId);
    image.value = null;
    emit('imagesUpdated');
  } catch (err) {
