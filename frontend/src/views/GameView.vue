      ...
      <table v-if="games.length">
        ...
        <tbody>
          <tr v-for="game in games" :key="game.id">
            <td>
              <a href="#" class="edit-link" @click.prevent="openEdit(game)">{{ game.name }}</a>
            </td>
...
export default {
    ...
    return {
      ...
      gameFields: [
        ...
        },
        {
          ...
          type: 'textarea',
          rows: 4,
          placeholder: 'Game description that appears on the join game turn sheet',
        },
      ],
    }
  },
  ...
  methods: {
    formatDate(dateStr) {
      ...
      return formatDateTime(dateStr, { timezone: authStore.accountTimezone })
    },
    ...
    checkCanCreateGame() {
      const basicSub = this.accountSubscriptions.find(
        (sub) => sub.subscription_type === 'basic_game_designer' && sub.status === 'active',
      )
      const professionalSub = this.accountSubscriptions.find(
        (sub) => sub.subscription_type === 'professional_game_designer' && sub.status === 'active',
      )
    ...
    async handleSubmit(formData) {
      ...
      try {
        const data = {
          ...
          turn_duration_hours:
            typeof formData.turn_duration_hours === 'string'
              ? parseInt(formData.turn_duration_hours, 10)
              : formData.turn_duration_hours,
