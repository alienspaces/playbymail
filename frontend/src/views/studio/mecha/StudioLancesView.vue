<template>
  <div>
        ...
        <template #cell-name="{ row }">
          <span class="name-cell">
            <a href="#" class="expand-link" @click.prevent="toggleMechs(row)">
              <span class="expand-icon">{{ expandedLanceId === row.id ? '▾' : '▸' }}</span>
              {{ row.name }}
            </a>
            <span v-if="row.lance_type === 'starter'" class="starter-badge" title="Player Starter Lance">Starter</span>
          </span>
        </template>
        <template #cell-lance_type="{ row }">
          {{ row.lance_type === 'starter' ? 'Starter' : 'Opponent' }}
        </template>
        <template #actions="{ row }">
          <TableActions :actions="getActions(row)" />
        </template>
        <template #row-detail="{ row }">
          <div v-if="expandedLanceId === row.id" class="mech-panel">
            ...
            <div v-else-if="mechsStore.getMechsForLance(row.id).length === 0" class="mech-empty">
              No mechs assigned to this lance.
            </div>
            <table v-else class="mech-table">
              ...
              <tbody>
                <tr v-for="mech in mechsStore.getMechsForLance(row.id)" :key="mech.id">
                  <td>{{ mech.callsign }}</td>
        ...
        <div class="modal">
          ...
          <form @submit.prevent="handleSubmit(modalForm)" class="modal-form">
            ...
            <div class="form-group">
              ...
              <select v-model="modalForm.lance_type" required>
                <option value="opponent">Opponent</option>
                <option value="starter">Starter</option>
              </select>
              <p class="field-hint">
                <span v-if="modalForm.lance_type === 'starter'">This lance is cloned for every player who joins the game. Only one starter lance is allowed per game.</span>
                <span v-else>This lance template is randomly assigned to a computer opponent when the game starts.</span>
              </p>
            </div>
    ...
    <ConfirmationModal :visible="showMechDeleteModal" title="Delete Mech"
      :message="`Are you sure you want to remove '${mechToDelete?.callsign}' from this lance?`"
...
import { useMechaLanceMechsStore } from '../../../stores/mechaLanceMechs'
...
const mechsStore = useMechaLanceMechsStore()
...
const columns = [
  { key: 'name', label: 'Name' },
  { key: 'lance_type', label: 'Type' },
  { key: 'description', label: 'Description' },
]

const showModal = ref(false)
const modalMode = ref('create')
const modalForm = ref({ name: '', description: '', lance_type: 'opponent' })
const modalError = ref('')
const showDeleteModal = ref(false)
const toDelete = ref(null)

// Lance mech sub-panel state
const expandedLanceId = ref(null)
const showMechModal = ref(false)
const mechModalMode = ref('create')
const mechModalForm = ref({ callsign: '', mecha_chassis_id: '', lance_id: '' })
const mechModalError = ref('')
const showMechDeleteModal = ref(false)
const mechToDelete = ref(null)
const activeLanceForMech = ref(null)

watch(() => selectedGame.value, (g) => {
  if (g) {
    store.fetchLances(g.id)
...
function openCreate() {
  ...
  modalForm.value = { name: '', description: '', lance_type: 'opponent' }
  modalError.value = ''
...
async function handleSubmit(formData) {
  ...
  const data = {
    ...
    lance_type: formData.lance_type,
  }
  try {
...
// Lance mech sub-panel functions
async function toggleMechs(row) {
  if (expandedLanceId.value === row.id) {
    expandedLanceId.value = null
  } else {
    expandedLanceId.value = row.id
    mechsStore.gameId = selectedGame.value.id
    await mechsStore.fetchLanceMechs(selectedGame.value.id, row.id)
  }
}

function openMechCreate(lance) {
  activeLanceForMech.value = lance
  mechModalMode.value = 'create'
  mechModalForm.value = { callsign: '', mecha_chassis_id: '', lance_id: lance.id }
  mechModalError.value = ''
  showMechModal.value = true
}

function openMechEdit(lance, mech) {
  activeLanceForMech.value = lance
  mechModalMode.value = 'edit'
  mechModalForm.value = { ...mech, lance_id: lance.id }
  mechModalError.value = ''
  showMechModal.value = true
}
...
async function handleMechSubmit(formData) {
  mechModalError.value = ''
  const lanceId = activeLanceForMech.value.id
  const data = {
  ...
  try {
    if (mechModalMode.value === 'create') {
      await mechsStore.createLanceMech(lanceId, data)
    } else {
      await mechsStore.updateLanceMech(lanceId, formData.id, data)
    }
    closeMechModal()
  } catch (e) {
...
function confirmMechDelete(lance, mech) {
  activeLanceForMech.value = lance
...
async function handleMechDelete() {
  try {
    await mechsStore.deleteLanceMech(activeLanceForMech.value.id, mechToDelete.value.id)
