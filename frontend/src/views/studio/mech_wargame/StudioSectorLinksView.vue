        ...
        <div class="modal">
          ...
          <form @submit.prevent="handleSubmit(modalForm)" class="modal-form">
            <div class="form-group">
              ...
              <select v-model="modalForm.from_mech_wargame_sector_id" required>
            ...
            <div class="form-group">
              ...
              <select v-model="modalForm.to_mech_wargame_sector_id" required>
                <option value="">-- Select sector --</option>
                <option v-for="s in sectorsStore.sectors" :key="s.id" :value="s.id">{{ s.name }}</option>
              </select>
            </div>
...
import { useMechWargameSectorLinksStore } from '../../../stores/mechWargameSectorLinks'
import { useMechWargameSectorsStore } from '../../../stores/mechWargameSectors'
...
const store = useMechWargameSectorLinksStore()
const sectorsStore = useMechWargameSectorsStore()
...
const formattedLinks = computed(() =>
  store.sectorLinks.map(sl => ({
    ...sl,
    from_sector_name: sectorName(sl.from_mech_wargame_sector_id),
    to_sector_name: sectorName(sl.to_mech_wargame_sector_id),
  }))
)
...
const modalForm = ref({ from_mech_wargame_sector_id: '', to_mech_wargame_sector_id: '', cover_modifier: 0 })
...
function openCreate() {
  ...
  modalForm.value = { from_mech_wargame_sector_id: '', to_mech_wargame_sector_id: '', cover_modifier: 0 }
  modalError.value = ''
...
async function handleSubmit(formData) {
  modalError.value = ''
  const allowed = ['from_mech_wargame_sector_id', 'to_mech_wargame_sector_id', 'cover_modifier']
