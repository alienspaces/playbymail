...
import {
  fetchWeapons as apiFetchWeapons,
  createWeapon as apiCreateWeapon,
  updateWeapon as apiUpdateWeapon,
  deleteWeapon as apiDeleteWeapon,
} from '../api/mechWargameWeapons'

export const useMechWargameWeaponsStore = defineStore('mechWargameWeapons', {
  state: () => ({
    weapons: [],
    loading: false,
    error: null,
  ...
  actions: {
    async fetchWeapons(gameId, pageNumber = 1) {
      this.loading = true
      this.error = null
      this.gameId = gameId
      try {
    ...
    async updateWeapon(weaponId, data) {
      this.loading = true
      this.error = null
      try {
