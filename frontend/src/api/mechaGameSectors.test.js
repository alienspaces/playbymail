...
import { fetchMechaGameSectors, createMechaGameSector, updateMechaGameSector, deleteMechaGameSector } from './mechaGameSectors'

describe('mechaGameSectors API', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockHandleApiError.mockImplementation((res) => res)
  })

  describe('fetchMechaGameSectors', () => {
    it('calls GET /api/v1/mecha-games/:gameId/sectors and returns data with hasMore', async () => {
      ...
      const result = await fetchMechaGameSectors('game-1')
    ...
    it('encodes gameId in URL', async () => {
      ...
      await fetchMechaGameSectors('game/id')
      expect(mockApiFetch.mock.calls[0][0]).toContain('game%2Fid')
    })
  })

  describe('createMechaGameSector', () => {
    it('sends POST with data and returns json.data', async () => {
      ...
      const result = await createMechaGameSector('game-1', data)
  ...
  describe('updateMechaGameSector', () => {
    it('sends PUT with data and returns json.data', async () => {
      ...
      const result = await updateMechaGameSector('game-1', 's1', data)
  ...
  describe('deleteMechaGameSector', () => {
    it('sends DELETE request', async () => {
      mockApiFetch.mockResolvedValue({ ok: true })

      await deleteMechaGameSector('game-1', 's1')
