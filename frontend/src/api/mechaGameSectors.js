...
const BASE = (gameId) => `${baseUrl}/api/v1/mecha-games/${encodeURIComponent(gameId)}/sectors`

export async function fetchMechaGameSectors(gameId, params = {}) {
...
export async function createMechaGameSector(gameId, data) {
...
export async function updateMechaGameSector(gameId, sectorId, data) {
...
export async function deleteMechaGameSector(gameId, sectorId) {
  const res = await apiFetch(`${BASE(gameId)}/${encodeURIComponent(sectorId)}`, {
    method: 'DELETE',
