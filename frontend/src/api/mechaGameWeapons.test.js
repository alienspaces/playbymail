...
import { fetchMechaGameWeapons, createMechaGameWeapon, updateMechaGameWeapon, deleteMechaGameWeapon } from './mechaGameWeapons'

describe('mechaGameWeapons API', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockHandleApiError.mockImplementation((res) => res)
  })

  describe('fetchMechaGameWeapons', () => {
    it('calls GET /api/v1/mecha-games/:gameId/weapons and returns data with hasMore', async () => {
      ...
      const result = await fetchMechaGameWeapons('game-1')
    ...
    it('encodes gameId in URL', async () => {
      ...
      await fetchMechaGameWeapons('game/id')
      expect(mockApiFetch.mock.calls[0][0]).toContain('game%2Fid')
    })
  })

  describe('createMechaGameWeapon', () => {
    it('sends POST with data and returns json.data', async () => {
      ...
      const result = await createMechaGameWeapon('game-1', data)
  ...
  describe('updateMechaGameWeapon', () => {
    it('sends PUT with data and returns json.data', async () => {
      ...
      const result = await updateMechaGameWeapon('game-1', 'w1', data)
  ...
  describe('deleteMechaGameWeapon', () => {
    it('sends DELETE request', async () => {
      mockApiFetch.mockResolvedValue({ ok: true })

      await deleteMechaGameWeapon('game-1', 'w1')
