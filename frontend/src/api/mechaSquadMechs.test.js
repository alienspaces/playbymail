...
import { fetchSquadMechs, createSquadMech, updateSquadMech, deleteSquadMech } from './mechaSquadMechs'

describe('mechaSquadMechs API', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockHandleApiError.mockImplementation((res) => res)
  })

  describe('fetchSquadMechs', () => {
    it('calls GET /api/v1/mecha-games/:gameId/squads/:squadId/mechs and returns data with hasMore', async () => {
      ...
      const result = await fetchSquadMechs('game-1', 'squad-1')
    ...
    it('encodes both gameId and squadId in URL', async () => {
      ...
      await fetchSquadMechs('game/id', 'squad/id')
  ...
  describe('createSquadMech', () => {
    it('sends POST with data and returns json.data', async () => {
      ...
      const result = await createSquadMech('game-1', 'squad-1', data)
  ...
  describe('updateSquadMech', () => {
    it('sends PUT with data and returns json.data', async () => {
      ...
      const result = await updateSquadMech('game-1', 'squad-1', 'm1', data)
  ...
  describe('deleteSquadMech', () => {
    it('sends DELETE request', async () => {
      mockApiFetch.mockResolvedValue({ ok: true })

      await deleteSquadMech('game-1', 'squad-1', 'm1')
