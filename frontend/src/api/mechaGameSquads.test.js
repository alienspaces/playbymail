...
import { fetchMechaGameSquads, createMechaGameSquad, updateMechaGameSquad, deleteMechaGameSquad } from './mechaGameSquads'

describe('mechaGameSquads API', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockHandleApiError.mockImplementation((res) => res)
  })

  describe('fetchMechaGameSquads', () => {
    it('calls GET /api/v1/mecha-games/:gameId/squads and returns data with hasMore', async () => {
      ...
      const result = await fetchMechaGameSquads('game-1')
    ...
    it('encodes gameId in URL', async () => {
      ...
      await fetchMechaGameSquads('game/id')
      expect(mockApiFetch.mock.calls[0][0]).toContain('game%2Fid')
    })
  })

  describe('createMechaGameSquad', () => {
    it('sends POST with data and returns json.data', async () => {
      ...
      const result = await createMechaGameSquad('game-1', data)
  ...
  describe('updateMechaGameSquad', () => {
    it('sends PUT with data and returns json.data', async () => {
      ...
      const result = await updateMechaGameSquad('game-1', 'l1', data)
  ...
  describe('deleteMechaGameSquad', () => {
    it('sends DELETE request', async () => {
      mockApiFetch.mockResolvedValue({ ok: true })

      await deleteMechaGameSquad('game-1', 'l1')
