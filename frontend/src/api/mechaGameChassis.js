...
const BASE = (gameId) => `${baseUrl}/api/v1/mecha-games/${encodeURIComponent(gameId)}/chassis`

export async function fetchMechaGameChassis(gameId, params = {}) {
...
export async function createMechaGameChassis(gameId, data) {
...
export async function updateMechaGameChassis(gameId, chassisId, data) {
...
export async function deleteMechaGameChassis(gameId, chassisId) {
  const res = await apiFetch(`${BASE(gameId)}/${encodeURIComponent(chassisId)}`, {
    method: 'DELETE',
