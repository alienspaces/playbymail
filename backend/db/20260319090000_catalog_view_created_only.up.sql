...
-- Revert catalog to show only 'created' game instances.
-- A 'started' game is already in progress; late-joining is not yet a supported feature.
-- Each test run creates fresh 'created' instances via db-load-test-data so the catalog
-- will always show joinable instances without showing in-progress games.
