...
SELECT DISTINCT ON (
    ...
    CASE
        WHEN gi.required_player_count = 0 THEN 0
        ELSE gi.required_player_count - COALESCE(pc.player_count, 0)
...
WHERE gi.status = 'created'
  ...
  AND (gi.required_player_count = 0
       OR COALESCE(pc.player_count, 0) < gi.required_player_count)
ORDER BY
    gs.account_id,
    g.id,
    gi.turn_duration_hours,
