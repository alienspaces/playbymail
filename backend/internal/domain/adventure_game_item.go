...
func (m *Domain) GetManyAdventureGameItemRecs(opts *coresql.Options) ([]*adventure_game_record.AdventureGameItem, error) {
...
func (m *Domain) GetAdventureGameItemRec(recID string, lock *coresql.Lock) (*adventure_game_record.AdventureGameItem, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, coreerror.NewNotFoundError(adventure_game_record.TableAdventureGameItem, recID)
	} else if err != nil {
		return nil, databaseError(err)
	}
	return rec, nil
...
func (m *Domain) UpdateAdventureGameItemRec(rec *adventure_game_record.AdventureGameItem) (*adventure_game_record.AdventureGameItem, error) {
	...
	currRec, err := m.GetAdventureGameItemRec(rec.ID, coresql.ForUpdateNoWait)
	if err != nil {
		return rec, err
	}
	...
	if err := m.validateAdventureGameItemRecForUpdate(currRec, rec); err != nil {
		l.Warn("failed to validate adventure_game_item record >%v<", err)
		return rec, err
	}
