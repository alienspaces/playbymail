package game_character_instance_test
...
func TestCreateOne(t *testing.T) {
	...
	tests := []struct {
		name   string
		rec    func(d harness.Data, t *testing.T) *record.GameCharacterInstance
		hasErr bool
	}{
		{
			name: "Without ID",
			rec: func(d harness.Data, t *testing.T) *record.GameCharacterInstance {
				...
				require.NoError(t, err)
				return &record.GameCharacterInstance{
					GameID:                 gameInstanceRec.GameID,
					GameInstanceID:         gameInstanceRec.ID,
					GameCharacterID:        charRec.ID,
					GameLocationInstanceID: locationInstanceRec.ID,
					Health:                 100,
				}
			},
			hasErr: false,
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			r := h.Domain.(*domain.Domain).GameCharacterInstanceRepository()
			require.NotNil(t, r)
