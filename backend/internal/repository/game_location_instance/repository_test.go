package game_location_instance_test
...
func TestCreateOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			rec: func(data *harness.Data, t *testing.T) *record.GameLocationInstance {
				...
				require.NoError(t, err)
				return &record.GameLocationInstance{
					GameID:         game.ID,
					GameInstanceID: gameInstance.ID,
					GameLocationID: location.ID,
				}
			},
			hasErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := h.Domain.(*domain.Domain).GameLocationInstanceRepository()
			rec := tt.rec(&h.Data, t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hasErr {
				...
				require.NoError(t, err)
				require.Equal(t, rec.GameLocationID, updated.GameLocationID)
			}
		})
	}
}
