...
func (t *Testing) createGameCharacterInstanceRec(cfg GameCharacterInstanceConfig, gameInstanceRec *record.GameInstance) (*record.GameCharacterInstance, error) {
	...
	var rec *record.GameCharacterInstance
	if cfg.Record != nil {
		recCopy := *cfg.Record
		rec = &recCopy
	} else {
		rec = &record.GameCharacterInstance{}
	}
	...
	rec.GameCharacterID = characterRec.ID
	...
	createdRec, err := t.Domain.(*domain.Domain).CreateGameCharacterInstanceRec(rec)
	if err != nil {
...
func (t *Testing) applyGameCharacterInstanceRecDefaultValues(rec *record.GameCharacterInstance) *record.GameCharacterInstance {
	if rec == nil {
		rec = &record.GameCharacterInstance{}
	}
	rec.Health = 100
