...
// MechaGameSquadManagementProcessor handles processing and creation of squad
// management turn sheets.
type MechaGameSquadManagementProcessor struct {
	Logger logger.Logger
	Domain *domain.Domain
	Config config.Config
}

// NewMechaGameSquadManagementProcessor creates a new management processor.
func NewMechaGameSquadManagementProcessor(l logger.Logger, d *domain.Domain, cfg config.Config) *MechaGameSquadManagementProcessor {
	return &MechaGameSquadManagementProcessor{Logger: l, Domain: d, Config: cfg}
}

// GetSheetType returns the sheet type this processor handles.
func (p *MechaGameSquadManagementProcessor) GetSheetType() string {
	return mecha_game_record.MechaGameTurnSheetTypeSquadManagement
}
...
func (p *MechaGameSquadManagementProcessor) ProcessTurnSheetResponse(
	ctx context.Context,
	...
	l := p.Logger.WithFunctionContext("MechaGameSquadManagementProcessor/ProcessTurnSheetResponse")

	l.Info("processing management sheet >%s< for squad instance >%s<", turnSheet.ID, squadInstance.ID)

	if turnSheet.SheetType != mecha_game_record.MechaGameTurnSheetTypeSquadManagement {
		return fmt.Errorf("invalid sheet type: expected %s, got %s",
			mecha_game_record.MechaGameTurnSheetTypeSquadManagement, turnSheet.SheetType)
	}

	var scanData turnsheet.SquadManagementScanData
	...
	// Reload squad instance for supply point mutation
	freshSquad, err := p.Domain.GetMechaGameSquadInstanceRec(squadInstance.ID, nil)
	if err != nil {
		return fmt.Errorf("failed to reload squad instance: %w", err)
	}
	...
	for _, order := range scanData.MechManagementOrders {
		...
		mechInst, err := p.Domain.GetMechaGameMechInstanceRec(order.MechInstanceID, nil)
		if err != nil {
		...
		if order.RepairStructure {
			chassisRec, err := p.Domain.GetMechaGameChassisRec(mechInst.MechaGameChassisID, nil)
		...
		if len(order.WeaponSwaps) > 0 {
			...
			for _, swap := range order.WeaponSwaps {
				...
				newWeapon, err := p.Domain.GetMechaGameWeaponRec(swap.NewWeaponID, nil)
				if err != nil {
		...
		if mechChanged {
			if _, err := p.Domain.UpdateMechaGameMechInstanceRec(mechInst); err != nil {
	...
	if spCost > 0 {
		freshSquad.SupplyPoints -= spCost
		if freshSquad.SupplyPoints < 0 {
			freshSquad.SupplyPoints = 0
		}
		events = append(events, turnsheet.TurnEvent{
			Category: turnsheet.TurnEventCategorySystem,
			Icon:     turnsheet.TurnEventIconSystem,
	...
	for _, evt := range events {
		if err := turnsheet.AppendMechaGameTurnEvent(freshSquad, evt); err != nil {
			l.Warn("failed to append management event for squad >%s<: %v", squadInstance.ID, err)
		}
	}

	if _, err := p.Domain.UpdateMechaGameSquadInstanceRec(freshSquad); err != nil {
		l.Warn("failed to update squad instance after management: %v", err)
	}
...
func (p *MechaGameSquadManagementProcessor) mechIsAtDepot(
	mechInst *mecha_game_record.MechaGameMechInstance,
) (bool, error) {
	sectorInst, err := p.Domain.GetMechaGameSectorInstanceRec(mechInst.MechaGameSectorInstanceID, nil)
	if err != nil {
...
// in this squad is at a depot sector. Returns nil (no sheet) otherwise.
// AI-controlled squads (no GameSubscriptionInstanceID) are skipped.
func (p *MechaGameSquadManagementProcessor) CreateNextTurnSheet(
	ctx context.Context,
	gameInstanceRec *game_record.GameInstance,
	squadInstance *mecha_game_record.MechaGameSquadInstance,
) (*game_record.GameTurnSheet, error) {
	l := p.Logger.WithFunctionContext("MechaGameSquadManagementProcessor/CreateNextTurnSheet")
	...
	// Only create management sheets for player-owned squads (those with a subscription instance)
	if !squadInstance.GameSubscriptionInstanceID.Valid {
		l.Info("squad instance >%s< has no subscription — skipping management sheet (AI squad)", squadInstance.ID)
		return nil, nil
	}
	_ = squadRec
	...
	if !anyAtDepot {
		l.Info("no mechs at depot for squad >%s< — skipping management sheet", squadInstance.ID)
		return nil, nil
	}

	return p.buildManagementSheet(ctx, l, gameInstanceRec, squadRec, squadInstance, mechInstances)
}

func (p *MechaGameSquadManagementProcessor) buildManagementSheet(
	_ context.Context,
	l logger.Logger,
	gameInstanceRec *game_record.GameInstance,
	squadRec *mecha_game_record.MechaGameSquad,
	squadInstance *mecha_game_record.MechaGameSquadInstance,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get account user for squad instance >%s<: %w", squadInstance.ID, err)
	}

	allWeapons, err := p.Domain.GetManyMechaGameWeaponRecs(&coresql.Options{
		Params: []coresql.Param{
			{Col: mecha_game_record.FieldMechaGameWeaponGameID, Val: gameRec.ID},
		},
	})
	if err != nil {
	...
	bgImageURL, err := p.Domain.GetGameTurnSheetImageDataURL(gameRec.ID, mecha_game_record.MechaGameTurnSheetTypeSquadManagement)
	if err == nil && bgImageURL != "" {
		backgroundImage = &bgImageURL
	}

	sheetData := turnsheet.SquadManagementData{
		...
		SupplyPoints:  squadInstance.SupplyPoints,
		Mechs:         mechEntries,
	...
	sheetOrder := mecha_game_record.MechaGameSheetPresentationOrderForType(mecha_game_record.MechaGameTurnSheetTypeSquadManagement)
	...
	gameTurnSheet := &game_record.GameTurnSheet{
		...
		TurnNumber:       turnNumber,
		SheetType:        mecha_game_record.MechaGameTurnSheetTypeSquadManagement,
	...
	// Link the game_turn_sheet to the squad instance via mecha_game_turn_sheet
	if _, err := p.Domain.CreateMechaGameTurnSheetRec(&mecha_game_record.MechaGameTurnSheet{
		GameID:               gameRec.ID,
	...
	l.Info("created management turn sheet >%s< for squad >%s< turn >%d<",
		sheetRec.ID, squadInstance.ID, turnNumber)
	return sheetRec, nil
}
