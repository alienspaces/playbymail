...
// Computer-opponent lances produce no turn sheets; their orders are generated by the
// decision engine during turn processing instead.
func (p *Mecha) createLanceTurnSheets(ctx context.Context, gameInstanceRec *game_record.GameInstance, lanceInstance *mecha_record.MechaLanceInstance) ([]*game_record.GameTurnSheet, error) {
	l := p.Logger.WithFunctionContext("Mecha/createLanceTurnSheets")

	l.Debug("creating turn sheets for lance instance >%s< turn number >%d<", lanceInstance.ID, gameInstanceRec.CurrentTurn)

	// Look up the parent lance to determine ownership type.
...
func (p *Mecha) createTurnSheet(ctx context.Context, gameInstanceRec *game_record.GameInstance, lanceInstance *mecha_record.MechaLanceInstance, turnSheetType string) (*game_record.GameTurnSheet, error) {
