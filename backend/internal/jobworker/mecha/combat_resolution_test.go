...
func TestWeaponCanFire(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "long range, same sector", rangeBand: mecha_record.WeaponRangeBandLong, distance: 0, expected: false},
		...
		{name: "long range, 2 sectors", rangeBand: mecha_record.WeaponRangeBandLong, distance: 2, expected: true},
	}
...
func TestApplyPendingDamage(t *testing.T) {
	...
	t.Run("damage absorbed fully by armor", func(t *testing.T) {
		t.Parallel()
		inst := &mecha_record.MechaMechInstance{
		...
		assert.Equal(t, mecha_record.MechInstanceStatusOperational, inst.Status)
	})
	...
	t.Run("damage overflows armor into structure causes damaged status", func(t *testing.T) {
		...
		assert.Equal(t, mecha_record.MechInstanceStatusDamaged, inst.Status)
	})
