...
func (p *Mecha) ProcessTurnSheets(ctx context.Context, gameInstanceRec *game_record.GameInstance) error {
	...
	if err := p.resolveCombat(ctx, l, gameInstanceRec, p.pendingAttacks); err != nil {
		l.Warn("failed to resolve combat >%v< — continuing (non-fatal)", err)
	}
	p.pendingAttacks = nil

	// Run end-of-turn lifecycle (heat dissipation, auto-repair, supply accrual)
	if err := p.runEndOfTurn(ctx, l, gameInstanceRec); err != nil {
...
func (p *Mecha) processComputerOpponentOrders(ctx context.Context, gameInstanceRec *game_record.GameInstance) error {
	...
	for _, opponentRec := range opponentRecs {
		// Find the lances owned by this opponent.
		opponentLances, err := p.Domain.GetManyMechaLanceRecs(&coresql.Options{
			Params: []coresql.Param{
				{Col: mecha_record.FieldMechaLanceMechaComputerOpponentID, Val: opponentRec.ID},
			},
		})
		if err != nil {
			l.Warn("failed to get lances for opponent >%s< >%v<", opponentRec.ID, err)
			continue
		}

		for _, lanceRec := range opponentLances {
			// Find the corresponding instance.
			var lanceInstance *mecha_record.MechaLanceInstance
			for _, li := range allLanceInstances {
				if li.MechaLanceID == lanceRec.ID {
					lanceInstance = li
					break
				}
			}
			if lanceInstance == nil {
				l.Warn("no lance instance found for computer opponent lance >%s< — skipping", lanceRec.ID)
				continue
			}
			...
			if err != nil {
				l.Warn("decision engine failed for opponent >%s< lance >%s<: %v", opponentRec.Name, lanceRec.Name, err)
				continue
			}
			...
			l.Info("applied %d orders for computer opponent >%s< lance >%s<", len(orders), opponentRec.Name, lanceRec.Name)
		}
	}
...
// the decision engine using the same logic as the human orders processor.
