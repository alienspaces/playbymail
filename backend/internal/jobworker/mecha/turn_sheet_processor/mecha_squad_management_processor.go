...
// MechaSquadManagementProcessor handles processing and creation of squad
...
// NewMechaSquadManagementProcessor creates a new management processor.
func NewMechaSquadManagementProcessor(l logger.Logger, d *domain.Domain, cfg config.Config) *MechaSquadManagementProcessor {
	return &MechaSquadManagementProcessor{Logger: l, Domain: d, Config: cfg}
}

// GetSheetType returns the sheet type this processor handles.
func (p *MechaSquadManagementProcessor) GetSheetType() string {
	return mecha_record.MechaTurnSheetTypeSquadManagement
}
...
func (p *MechaSquadManagementProcessor) ProcessTurnSheetResponse(
	ctx context.Context,
	gameInstanceRec *game_record.GameInstance,
	squadInstance *mecha_record.MechaSquadInstance,
	turnSheet *game_record.GameTurnSheet,
) error {
	l := p.Logger.WithFunctionContext("MechaSquadManagementProcessor/ProcessTurnSheetResponse")

	l.Info("processing management sheet >%s< for squad instance >%s<", turnSheet.ID, squadInstance.ID)

	if turnSheet.SheetType != mecha_record.MechaTurnSheetTypeSquadManagement {
		return fmt.Errorf("invalid sheet type: expected %s, got %s",
			mecha_record.MechaTurnSheetTypeSquadManagement, turnSheet.SheetType)
	}
	...
	for _, evt := range events {
		if err := turnsheet.AppendMechaTurnEvent(freshSquad, evt); err != nil {
...
func (p *MechaSquadManagementProcessor) mechIsAtDepot(
...
func (p *MechaSquadManagementProcessor) CreateNextTurnSheet(
	ctx context.Context,
	gameInstanceRec *game_record.GameInstance,
	squadInstance *mecha_record.MechaSquadInstance,
) (*game_record.GameTurnSheet, error) {
	l := p.Logger.WithFunctionContext("MechaSquadManagementProcessor/CreateNextTurnSheet")
...
func (p *MechaSquadManagementProcessor) buildManagementSheet(
	_ context.Context,
	l logger.Logger,
	gameInstanceRec *game_record.GameInstance,
	squadRec *mecha_record.MechaSquad,
	squadInstance *mecha_record.MechaSquadInstance,
	...
	bgImageURL, err := p.Domain.GetGameTurnSheetImageDataURL(gameRec.ID, mecha_record.MechaTurnSheetTypeSquadManagement)
	...
	sheetOrder := mecha_record.MechaSheetPresentationOrderForType(mecha_record.MechaTurnSheetTypeSquadManagement)
	...
	gameTurnSheet := &game_record.GameTurnSheet{
		...
		TurnNumber:       turnNumber,
		SheetType:        mecha_record.MechaTurnSheetTypeSquadManagement,
	...
	// Link the game_turn_sheet to the squad instance via mecha_turn_sheet
