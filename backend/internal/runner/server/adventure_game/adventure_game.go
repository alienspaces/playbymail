...
func AdventureGameHandlerConfig(cfg config.Config, l logger.Logger, scnr turnsheet.TurnSheetScanner) (map[string]server.HandlerConfig, error) {
	...
	handlerConfigFuncs := []func(logger.Logger) (map[string]server.HandlerConfig, error){
		...
		adventureGameLocationObjectStateHandlerConfig,
	}
