	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.Name, func(t *testing.T) {
			testFunc := func(method string, body interface{}) {
				...
				var responses []*schema.GameLocationResponseData
				if testCase.collectionRequest {
					responses = body.(schema.GameLocationCollectionResponse).Data
				} else {
					responses = append(responses, body.(schema.GameLocationResponse).Data)
				}
