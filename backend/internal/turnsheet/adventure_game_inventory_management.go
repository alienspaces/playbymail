...
// GeneratePreviewData generates dummy data for an inventory management turn sheet preview
func (p *InventoryManagementProcessor) GeneratePreviewData(ctx context.Context, l logger.Logger, gameRec *game_record.Game, backgroundImage *string) ([]byte, error) {
	l = l.WithFunctionContext("InventoryManagementProcessor/GeneratePreviewData")

	l.Info("generating inventory management preview data")
	...
	inventoryData := InventoryManagementData{
		TurnSheetTemplateData: TurnSheetTemplateData{
			...
			TurnSheetDescription:  convert.Ptr("Manage your inventory and equipment."),
			TurnSheetInstructions: convert.Ptr(DefaultInventoryManagementInstructions()),
			TurnSheetDeadline:     nil,
			TurnSheetCode:         convert.Ptr(turnSheetCode),
		},
		CharacterName:       "Example Hero",
		CurrentLocationName: "Starting Village",
		InventoryCapacity:   10,
		InventoryCount:      3,
		CurrentInventory: []InventoryItem{
			{
				ItemInstanceID:  record.NewRecordID(),
				ItemName:        "Rusty Sword",
				ItemDescription: "A slightly rusted iron sword.",
				IsEquipped:      true,
			...
			},
			{
				...
				ItemDescription: "Basic leather armor.",
				IsEquipped:      true,
				EquipmentSlot:   "armor",
				CanEquip:        true,
			},
		...
		LocationItems: []LocationItem{
			{
				...
				ItemDescription: "A sturdy wooden shield.",
			},
		},
	}
	...
	return json.Marshal(inventoryData)
}
