...
func GetTurnSheetJoinGameData(gameRec *game_record.Game, turnSheetCode string) (JoinGameData, error) {
	...
	case game_record.GameTypeMecha:
		return createMechaGameJoinGameData(gameRec, turnSheetCode), nil
	default:
...
// Turn sheet codes are base64 URL-encoded semicolon-separated values (typically 100+ characters)
func (bp *BaseProcessor) ExtractTurnSheetCode(text string) (string, error) {
	...
	if len(allCodes) > 0 {
		...
		for _, code := range allCodes {
			...
			// Strongly prefer longer codes - base64 encoded semicolon-separated values are typically 100+ chars
			if len(code) >= 100 {
				score += 100
			} else if len(code) >= 50 {
				score += 50
			...
			// Check if it looks like valid base64 encoded turn sheet code
			// New format starts with "cGxheWluZw" (playing) or "am9pbmluZw" (joining) in base64
			// Old format started with "ey" (JSON "{" in base64) - kept for backward compatibility
			if strings.HasPrefix(code, "cGxheWluZw") || strings.HasPrefix(code, "am9pbmluZw") {
				score += 50 // New semicolon-separated format
			} else if strings.HasPrefix(code, "ey") {
				score += 50 // Legacy JSON format (backward compatibility)
			}
