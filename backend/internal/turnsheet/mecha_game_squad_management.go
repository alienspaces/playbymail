...
// Ensure MechaGameSquadManagementProcessor implements DocumentProcessor at compile time.
var _ DocumentProcessor = (*MechaGameSquadManagementProcessor)(nil)
...
const managementTemplatePath = "turnsheet/mecha_game_squad_management.template"
...
// MechaGameSquadManagementProcessor implements DocumentProcessor for management sheets.
type MechaGameSquadManagementProcessor struct {
	*BaseProcessor
}

// NewMechaGameSquadManagementProcessor creates a new management sheet processor.
func NewMechaGameSquadManagementProcessor(l logger.Logger, cfg config.Config) (*MechaGameSquadManagementProcessor, error) {
	baseProcessor, err := NewBaseProcessor(l, cfg)
	if err != nil {
		return nil, err
	}
	return &MechaGameSquadManagementProcessor{
		BaseProcessor: baseProcessor,
	}, nil
}

// GenerateTurnSheet generates a squad management turn sheet document.
func (p *MechaGameSquadManagementProcessor) GenerateTurnSheet(ctx context.Context, l logger.Logger, format DocumentFormat, sheetData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameSquadManagementProcessor/GenerateTurnSheet")
...
func (p *MechaGameSquadManagementProcessor) ScanTurnSheet(ctx context.Context, l logger.Logger, sheetData []byte, imageData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameSquadManagementProcessor/ScanTurnSheet")
...
func (p *MechaGameSquadManagementProcessor) GeneratePreviewData(ctx context.Context, l logger.Logger, gameRec *game_record.Game, backgroundImage *string) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameSquadManagementProcessor/GeneratePreviewData")
