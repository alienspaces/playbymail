...
// MechaOrdersProcessor implements the DocumentProcessor interface for mecha orders sheets.
type MechaOrdersProcessor struct {
	*BaseProcessor
}

// NewMechaOrdersProcessor creates a new mecha orders processor.
func NewMechaOrdersProcessor(l logger.Logger, cfg config.Config) (*MechaOrdersProcessor, error) {
	baseProcessor, err := NewBaseProcessor(l, cfg)
	if err != nil {
		return nil, err
	}
	return &MechaOrdersProcessor{
		BaseProcessor: baseProcessor,
	}, nil
}

// GeneratePreviewData generates dummy data for a mecha orders turn sheet preview.
func (p *MechaOrdersProcessor) GeneratePreviewData(ctx context.Context, l logger.Logger, gameRec *game_record.Game, backgroundImage *string) ([]byte, error) {
	l = l.WithFunctionContext("MechaOrdersProcessor/GeneratePreviewData")
...
func (p *MechaOrdersProcessor) GenerateTurnSheet(ctx context.Context, l logger.Logger, format DocumentFormat, sheetData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaOrdersProcessor/GenerateTurnSheet")
...
func (p *MechaOrdersProcessor) ScanTurnSheet(ctx context.Context, l logger.Logger, sheetData []byte, imageData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaOrdersProcessor/ScanTurnSheet")
...
func (p *MechaOrdersProcessor) resolveOrdersTemplateData(sheetData []byte) *OrdersData {
	var data OrdersData
	if len(sheetData) > 0 {
		if err := json.Unmarshal(sheetData, &data); err != nil {
