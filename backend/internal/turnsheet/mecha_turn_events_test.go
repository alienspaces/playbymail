...
func TestAppendMechaTurnEvent(t *testing.T) {
	...
	t.Run("appends to empty squad instance", func(t *testing.T) {
		t.Parallel()
		squad := &mecha_record.MechaSquadInstance{}

		err := turnsheet.AppendMechaTurnEvent(squad, turnsheet.TurnEvent{
		...
		events, err := turnsheet.ReadMechaTurnEvents(squad)
		require.NoError(t, err)
	...
	t.Run("appends to squad instance with existing events", func(t *testing.T) {
		...
		require.NoError(t, turnsheet.AppendMechaTurnEvent(squad, turnsheet.TurnEvent{
			Category: turnsheet.TurnEventCategoryMovement,
			Message:  "First event.",
		}))
		require.NoError(t, turnsheet.AppendMechaTurnEvent(squad, turnsheet.TurnEvent{
...
func TestReadMechaTurnEvents(t *testing.T) {
	...
	t.Run("does not modify the squad instance", func(t *testing.T) {
		...
		_, err = turnsheet.ReadMechaTurnEvents(squad)
		require.NoError(t, err)
...
func TestReadAndClearMechaTurnEvents(t *testing.T) {
	...
	t.Run("returns events and clears the field", func(t *testing.T) {
		...
		events, err := turnsheet.ReadAndClearMechaTurnEvents(squad)
		require.NoError(t, err)
		require.Len(t, events, 2)
		require.Equal(t, "Moved to sector A.", events[0].Message)

		// Field should now be empty
		remaining, err := turnsheet.ReadMechaTurnEvents(squad)
		require.NoError(t, err)
