...
func DefaultMechaGameJoinGameInstructions() string {
	return defaultMechaGameJoinGameInstructions
}

// MechaGameJoinGameScanData captures the fields extracted from a scanned mecha join turn sheet.
// It embeds the generic JoinGameScanData and adds mecha specific fields.
type MechaGameJoinGameScanData struct {
	JoinGameScanData
	CommanderName string `json:"commander_name"`
}

// Validate ensures required fields are present in the scanned data.
func (d *MechaGameJoinGameScanData) Validate() error {
...
// MechaGameJoinGameProcessor implements the DocumentProcessor interface for mecha join sheets.
type MechaGameJoinGameProcessor struct {
	*BaseProcessor
}

// NewMechaGameJoinGameProcessor creates a new mecha join game processor.
func NewMechaGameJoinGameProcessor(l logger.Logger, cfg config.Config) (*MechaGameJoinGameProcessor, error) {
	baseProcessor, err := NewBaseProcessor(l, cfg)
	if err != nil {
		return nil, err
	}
	return &MechaGameJoinGameProcessor{
		BaseProcessor: baseProcessor,
	}, nil
}

// createMechaGameJoinGameData creates join game data from a mecha record and turn sheet code.
func createMechaGameJoinGameData(gameRec *game_record.Game, turnSheetCode string) JoinGameData {
	return JoinGameData{
		TurnSheetTemplateData: TurnSheetTemplateData{
			...
			TurnSheetInstructions: convert.Ptr(DefaultMechaGameJoinGameInstructions()),
...
func (p *MechaGameJoinGameProcessor) GeneratePreviewData(ctx context.Context, l logger.Logger, gameRec *game_record.Game, backgroundImage *string) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameJoinGameProcessor/GeneratePreviewData")
	...
	turnSheetData := createMechaGameJoinGameData(gameRec, turnSheetCode)
...
func (p *MechaGameJoinGameProcessor) GenerateTurnSheet(ctx context.Context, l logger.Logger, format DocumentFormat, sheetData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameJoinGameProcessor/GenerateTurnSheet")
	...
	if data.TurnSheetInstructions == nil || strings.TrimSpace(*data.TurnSheetInstructions) == "" {
		instruction := defaultMechaGameJoinGameInstructions
	...
	return p.GenerateDocument(ctx, format, mechaGameJoinGameTemplatePath, &data)
}

// ScanTurnSheet extracts join game information from the uploaded document using the hosted scanner.
func (p *MechaGameJoinGameProcessor) ScanTurnSheet(ctx context.Context, l logger.Logger, sheetData []byte, imageData []byte) ([]byte, error) {
	l = l.WithFunctionContext("MechaGameJoinGameProcessor/ScanTurnSheet")
	...
	templateData := p.resolveMechaGameJoinGameTemplateData(sheetData)

	templateImage, err := p.renderTemplatePreview(ctx, mechaGameJoinGameTemplatePath, templateData)
	if err != nil {
	...
	req := scanner.StructuredScanRequest{
		Instructions:       buildMechaGameJoinGameInstructions(),
		AdditionalContext:  buildMechaGameJoinGameContext(templateData),
		TemplateImage:      templateImage,
		TemplateImageMIME:  "image/png",
		FilledImage:        imageData,
		ExpectedJSONSchema: mechaGameJoinGameExpectedSchema(),
	}
	...
	var scanData MechaGameJoinGameScanData
	...
	normalizeMechaGameJoinGameScanData(&scanData)
...
func (p *MechaGameJoinGameProcessor) resolveMechaGameJoinGameTemplateData(sheetData []byte) *JoinGameData {
	var data JoinGameData
	if len(sheetData) > 0 {
		if err := json.Unmarshal(sheetData, &data); err != nil {
			return defaultMechaGameJoinGameTemplateData()
		}
		return &data
	}
	return defaultMechaGameJoinGameTemplateData()
...
func defaultMechaGameJoinGameTemplateData() *JoinGameData {
	title := "Join Game"
	instructions := DefaultMechaGameJoinGameInstructions()
	return &JoinGameData{
		TurnSheetTemplateData: TurnSheetTemplateData{
...
func mechaGameJoinGameExpectedSchema() map[string]any {
...
func buildMechaGameJoinGameInstructions() string {
	return `You are comparing two images of a PlayByMail "Join Game" form for a mecha_game.
...
func buildMechaGameJoinGameContext(data *JoinGameData) []string {
	var ctx []string
	if data != nil {
...
func normalizeMechaGameJoinGameScanData(data *MechaGameJoinGameScanData) {
