...
// GeneratePreviewData generates dummy data for a join game turn sheet preview
func (p *JoinGameProcessor) GeneratePreviewData(ctx context.Context, l logger.Logger, gameRec *game_record.Game, backgroundImage *string) ([]byte, error) {
	l = l.WithFunctionContext("JoinGameProcessor/GeneratePreviewData")

	l.Info("generating join game preview data")
	...
	// Get the turn sheet data to populate the turn sheet content
	turnSheetData := createAdventureGameJoinGameData(gameRec, turnSheetCode)
...
func (p *JoinGameProcessor) GenerateTurnSheet(ctx context.Context, l logger.Logger, format DocumentFormat, sheetData []byte) ([]byte, error) {
	...
	return p.GenerateDocument(ctx, format, joinGameTemplatePath, &data)
}
