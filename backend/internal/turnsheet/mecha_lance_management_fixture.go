...
// MechaLanceManagementFixture returns the sample rendering fixture for the
// mecha lance management turn sheet.
func MechaLanceManagementFixture() DevFixture {
	return DevFixture{
		TemplatePath:   "turnsheet/mecha_lance_management.template",
		OutputBaseName: "mecha_lance_management_turnsheet",
		BackgroundFile: "background-darkforest.png",
		MakeData: func(bg, code string) any {
			deadline := time.Now().Add(7 * 24 * time.Hour)
			return &LanceManagementData{
				TurnSheetTemplateData: TurnSheetTemplateData{
					GameName:              strPtr("Steel Thunder"),
					GameType:              strPtr("mecha"),
					TurnNumber:            intPtr(2),
					TurnSheetTitle:        strPtr("Lance Management"),
					...
					TurnEvents: []TurnEvent{
						...
						{Category: TurnEventCategorySystem, Icon: TurnEventIconSystem, Message: "Lance received 2 supply points (6 total)."},
					},
				},
		...
		NewProcessor: func(l logger.Logger, cfg config.Config) (TurnSheetProcessor, error) {
			return NewMechaLanceManagementProcessor(l, cfg)
		},
	}
}
