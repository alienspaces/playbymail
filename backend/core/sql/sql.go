...
const (
	...
	OpOverlap          Operator = "&&"
	OpILikeAny         Operator = "ILIKE ANY"
	OpLikeAny          Operator = "LIKE ANY"
...
func (o *Options) GetParam(param string) []Param {
	params := []Param{}
...
func (o *Options) RemoveParam(param string) {
	params := o.Params
...
func From(initialSQL string, opts *Options) (string, pgx.NamedArgs, error) {
	if opts == nil {
	...
	for _, param := range opts.Params {
		...
		if ArrayOps.Has(op) && len(param.Array) == 0 {
		...
		case OpILikeAny, OpLikeAny:
			namedParams := toNamedArrayParams(param.Array, col)
			opClause += fmt.Sprintf("CAST(%s AS TEXT) %s (array[%s])", param.Col, op, namedParams)
