...
func Test_GetOneSQL(t *testing.T) {
	...
	expected := fmt.Sprintf("SELECT db_A, db_B, db_d, db_E, db_E3, db_E5, db_E6, db_f, db_time, id, created_at, updated_at, deleted_at FROM %s WHERE deleted_at IS NULL", r.tableName)
	require.Equal(t, expected, strings.TrimSpace(r.GetOneSQL()))
}
...
func Test_GetManySQL(t *testing.T) {
	...
	expected := fmt.Sprintf("SELECT db_A, db_B, db_d, db_E, db_E3, db_E5, db_E6, db_f, db_time, id, created_at, updated_at, deleted_at FROM %s WHERE deleted_at IS NULL", r.tableName)
	require.Equal(t, expected, strings.TrimSpace(r.GetManySQL()))
...
`, r.tableName))
	...
	require.Equal(t, expected, strings.TrimSpace(r.UpdateOneSQL()))
}
...
`, r.tableName))
	...
	require.Equal(t, expected, strings.TrimSpace(r.DeleteOneSQL()))
}
